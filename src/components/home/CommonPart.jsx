...
const SecondPart = ({ img, imagePosition, content }) => {
  return (
    <div
      className={`flex flex-col sm:flex-row items-center justify-center relative w-full sm:h-[650px] overflow-hidden mb-10 sm:mb-0 `}
    >
      <div
      ...
      >
        <img
          src={img}
          alt="coverImg"
          className="object-cover h-full hidden sm:block"
        />
      </div>
      <div className="w-full h-full max-w-[1280px] mx-auto z-10 flex flex-col sm:flex-row items-center justify-center">
        ...
        <div
          className={`sm:w-1/2 ${
            imagePosition === "left" ? "sm:order1 " : "sm:order-2"
          } hidden sm:block`}
        >
        ...
        <div className="sm:w-1/2 sm:p-20 flex flex-col items-center justify-center bg-white p-3">
          ...
          <div
            data-aos="fade-right"
            className={`${
              imagePosition === "left" ? "text-start" : "text-end"
            } hidden sm:block`}
          >
            <h1 className="font- text-3xl font-primaryFont tracking-wider uppercase">
              {content?.title}
            </h1>
            <h5 className="font-semibold text-[#2F2E2E] font-primaryFont mt-2 tracking-widest">
              {content?.subHeading}
            </h5>
            <p className="my-5 text-[14px] text-[#2F2E2E] leading-normal font-contentFont">
              {content?.description}
            </p>
            {content?.button === "true" && (
          ...
          {/* mobile view */}
          <div data-aos="fade-right" className={`sm:hidden`}>
            <h1 className="font-bold text-3xl uppercase">{content?.title}</h1>
